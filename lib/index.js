...
const getAccessToken = () => {
    return readdirSync(awsSsoCacheFolderPath).reduce((token, file) => {
        ...
        if (
            ...
            return {
                expiresAt: new Date(data.expiresAt.replace(/UTC/gm, `Z`)),
...
const getConfigForProfile = (config, profile) => {
    ...
    const profiles = Object.entries(config).reduce(
        (profiles, [key, value]) => {
            if (key.startsWith(prefix)) {
                profiles[key.substring(prefix.length)] = value
            }
            return profiles
        },
        {
    ...
    if (!profileConfig) {
        console.error(`The profile '${profile}' not found. Available profiles: ${Object.keys(profiles).map(name => `'${name}'`).join(', ')}`)
        return undefined
    }
    ...
    if (
        (
            ...
            typeof profileConfig.region === "undefined"
        ) ||
        ...
        console.error("\tsso_region (or region)")
        console.error("\tsso_account_id")
        console.error("\tsso_role_name")
        console.error(`Found: ${JSON.stringify(profileConfig, null, 2)}`)
...
export default async (profile, customProfile, force = false) => {
    try {
        ...
        let accessToken = force ? null : getAccessToken()

        if (accessToken === null || accessToken.expiresAt < new Date()) {
            await login(profile)
            accessToken = getAccessToken()
            if (accessToken === null) {
                throw new Error('No access token available available after login.')
            }
        }
        ...
        const { roleCredentials } = await sso.send(new GetRoleCredentialsCommand(params))
